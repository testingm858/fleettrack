...
// Attaches the bearer token automatically and transparently retries once
// after a silent refresh if the access token had expired — callers never
// deal with token lifecycle themselves.
...
// The export endpoints require the auth header, so a plain <a href> can't
// hit them directly — fetch as a blob and trigger the download ourselves.
export async function downloadAlertsExport(
  format: "csv" | "pdf",
  params: { from?: string; to?: string; vehicleId?: string; type?: AlertType; search?: string } = {},
): Promise<void> {
  const auth = getAuth();
  const query = new URLSearchParams();
  if (params.from) query.set("from", params.from);
  ...
  const res = await fetch(`${API_BASE_URL}/api/alerts/export.${format}${qs ? `?${qs}` : ""}`, {
  ...
  link.href = url;
  link.download = `alerts-export.${format}`;
  link.click();
